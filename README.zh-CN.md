...
|------|------|
| **一键下载整话** | 手动下载当前章节 |
| **自动检查：关/开** | 开启后，RSS 发现新话时自动触发下载 |
| **RSS轮询：关/开** | 开启后，定时拉取系列 RSS Feed 检查更新（默认每 1 分钟） |

- RSS 轮询每次仅请求几 KB 的 XML，高频轮询无压力。
- 自动下载仅处理近 24 小时内发布的新话（可在脚本顶部修改 `CONFIG.autoRecentWindowMs`）。
- 两个开关的状态通过 `localStorage` 持久化，刷新页面后保持。
- 轮询间隔可通过 `CONFIG.autoCheckIntervalMs` 调整。
